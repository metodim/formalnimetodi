...
/**
 * <p>
 * Produces a root {@link CcsOperation} after walking the CCS grammar Abstract
 * Syntax Tree (AST). This is a standard domain model generation phase from an
 * AST..
 * </p>
 * 
 * <p>
 * The generated {@link CcsOperation} represents the root of the CCS
 * expression..
 * </p>
 * 
...
public class ASTDomainBuilder {
	...
	/**
	 * Non recursive implementation for building the Ccs domain model tree form
	 * AST tree.
	 * 
	 * @param node
	...
	private CcsOperator getDomainNoRecursion(Tree rootNode) {
		...
		for (Tree node : nodeList) {
			...
			case RENAME:
				...
				break;
			case TAU:
				LOG.debug("TAU"); // Action = TAU | CO_LABEL | LABEL
			...
			case RESTRICT_LABELS:
				LOG.debug("RESTRICT_LABELS");
	...
	private boolean isNotRestricOrRename(Tree node) {
		return node.getType() != RESTRICT_LABELS && node.getType() != RENAME_CLAUSE && node.getType() != RENAME_SINGLE;
	}
	...
	class ListOperator extends CcsOperator {
		...
		public <T extends CcsOperator> void setList(List<T> list) {
			listOperator = list;
		}

		@SuppressWarnings("unchecked")
		public <T extends CcsOperator> List<T> getList() {
			return (List<T>) listOperator;
		}
	}
	...
	class MapOperator extends CcsOperator {
		...
		public <T extends CcsOperator> void setMap(Map<T, T> map) {
			mapOperator = map;
		}

		@SuppressWarnings("unchecked")
		public <T extends CcsOperator> Map<T, T> getMap() {
			return (Map<T, T>) mapOperator;
		}
	}

	/**
	 * Recursive implementation for building the Ccs domain model tree form AST
	 * tree.
	 * 
	 * @param node
	 *            - the root node of the AST tree
	...
	private CcsOperator getDomain(Tree node) {
		...
		case DEF:
			LOG.debug("DEF");
			return new CcsDefinition(new CcsProcess(node.getChild(0).getText()), (CcsOperation) getDomain((CommonTree) node.getChild(1)));

		case ADD:
			LOG.debug("ADD");
			return new CcsAdd((CcsOperation) getDomain(node.getChild(0)), (CcsOperation) getDomain((CommonTree) node.getChild(1)));

		case SYNC:
			LOG.debug("SYNC");
			return new CcsSynch((CcsOperation) getDomain(node.getChild(0)), (CcsOperation) getDomain(node.getChild(1)));

		case RESTRICT:
			LOG.debug("RESTRICT");
			return new CcsRestrict((CcsOperation) getDomain(node.getChild(0)), getRestrictActions(node.getChild(1)));

		case RENAME:
			LOG.debug("RENAME");
			return new CcsRename((CcsOperation) getDomain(node.getChild(0)), getRenameMap(node.getChild(1)));
		...
		case TRANSITION:
			LOG.debug("TRANSITION");
			return new CcsTrans((CcsAction) getDomain(node.getChild(0)), (CcsOperation) getDomain(node.getChild(1)));
	...
	private List<CcsAction> getRestrictActions(Tree node) {
		...
		for (int i = 0; i < node.getChildCount(); i++) {
			if (node.getChild(i).getType() != LABEL && node.getChild(i).getType() != CO_LABEL && node.getChild(i).getType() != TAU)
	...
	private Map<CcsAction, CcsAction> getRenameMap(Tree node) {
		...
		for (int i = 0; i < node.getChildCount(); i++) {
			...
			renameMap.put(new CcsAction(renameSingle.getChild(0).getText()), new CcsAction(renameSingle.getChild(1).getText()));
		}
	...
	private void toStringNode(Tree node) {
		...
		case LABEL:
			...
			break;
		case CO_LABEL:
			System.out.println("CO_LABEL");
			break;
		case PROCESS:
			System.out.println("PROCESS");
			break;
		case TRANSITION:
			System.out.println("ТРАNSITION");
			break;
		case RESTRICT_LABELS:
			System.out.println("RESTRICT_LABELS");
			break;
		case RENAME_CLAUSE:
			System.out.println("RENAME_CLAUSE");
			break;
		case RENAME_SINGLE:
			System.out.println("RENAME_SINGLE");
			break;
		}
	}
